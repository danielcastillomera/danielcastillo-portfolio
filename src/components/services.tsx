...
export default function Services() {
  ...
  return (
    <section id="servicios" className="relative py-24 sm:py-32" aria-labelledby="services-heading">
      ...
      >
        <div className="mb-16">
          ...
          <h2 id="services-heading" className="mt-3 text-3xl font-bold tracking-tight text-gray-900 dark:text-white sm:text-4xl" style={{ fontFamily: 'var(--font-display)' }}>
            {t.services.title}
          </h2>
          <div className="mt-3 h-1 w-16 rounded-full bg-gradient-to-r from-accent-500 to-orange-500" aria-hidden="true" />
          <p className="mt-4 text-base text-gray-800 dark:text-gray-100 max-w-2xl">{t.services.subtitle}</p>
        </div>
        ...
        {SERVICE_CATEGORIES.map((cat, catIdx) => (
          <div key={cat.title} className="mb-12 last:mb-0">
            <h3 className="text-lg font-bold text-gray-800 dark:text-white mb-6 flex items-center gap-3">
              <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-accent-500/10 text-accent-500 text-sm" aria-hidden="true">
                {catIdx === 0 ? '💻' : '🔧'}
              </span>
