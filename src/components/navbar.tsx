...
export default function Navbar() {
  ...
  return (
    <header
      role="banner"
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'glass shadow-lg shadow-black/5 dark:shadow-navy-950/50' : 'bg-transparent'}`}
    >
      <nav
        role="navigation"
      ...
      >
        {/* Logo */}
        <a href="#inicio" className="group flex items-center gap-2.5 transition-opacity hover:opacity-80" aria-label="Ir al inicio">
          ...
          <span className="hidden lg:inline text-sm font-bold text-gray-900 dark:text-white truncate max-w-[220px]">
            {PERSONAL.name}
          </span>
        </a>
          ...
          {NAV_LINKS.map((link) => {
            ...
            return (
              <li key={link.href}>
                <a
                  href={link.href}
                  className={`relative px-3 py-2 text-sm font-medium rounded-lg transition-all duration-300 ${isActive ? 'text-accent-500' : 'text-gray-600 dark:text-navy-200 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-white/5'}`}
        ...
        <div className="flex items-center gap-1.5">
          ...
          <a href="#contacto" className="hidden md:inline-flex items-center gap-2 rounded-lg bg-accent-500 px-4 py-2 text-sm font-semibold text-black transition-all hover:bg-accent-400 hover:shadow-lg hover:shadow-accent-500/20 active:scale-95">
            {t.nav.contactame}
          </a>
          ...
          <button
            type="button"
            className="relative z-50 flex lg:hidden h-10 w-10 items-center justify-center rounded-lg text-gray-700 dark:text-white hover:bg-gray-100 dark:hover:bg-white/10 transition-colors"
      ...
      <div
        id="mobile-menu"
        className={`fixed inset-0 z-40 lg:hidden transition-all duration-500 ${isOpen ? 'visible opacity-100' : 'invisible opacity-0'}`}
        aria-hidden={!isOpen}
      >
        ...
        <nav className="relative flex h-full flex-col items-center justify-center gap-3" aria-label="Menú móvil">
          ...
          <a
            href="#contacto"
            className={`mt-6 inline-flex items-center gap-2 rounded-xl bg-accent-500 px-8 py-3 text-base font-bold text-black transition-all duration-500 hover:bg-accent-400 ${isOpen ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}
