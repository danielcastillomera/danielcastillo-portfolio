  ...
  return (
    <section id="proyectos" className="relative py-24 sm:py-32" aria-labelledby="projects-heading">
      ...
      <div ref={sectionRef} className={`relative mx-auto max-w-7xl px-6 lg:px-8 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        ...
        {/* Featured */}
        {featured && (
          <article className="glass rounded-2xl overflow-hidden card-glow mb-12" aria-label={`${t.projects.individual}: ${featured.title}`}>
            <div className="flex items-center justify-between border-b border-gray-100 dark:border-navy-700/50 px-6 py-4 sm:px-8">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="rounded-md bg-gray-100 dark:bg-navy-700/60 px-2.5 py-1 text-xs font-mono text-gray-800 dark:text-gray-100">
              ...
              <div className="grid gap-8 lg:grid-cols-2 lg:gap-12">
                <div className="space-y-5">
                  <div>
                    ...
                    <h3 className="mt-2 text-2xl font-bold text-gray-900 dark:text-white sm:text-3xl" style={{ fontFamily: 'var(--font-display)' }}>
                      {featured.title}
                    </h3>
                  </div>
            ...
            {others.map((project) => (
              <article key={project.id} className="glass rounded-xl p-6 card-glow" aria-label={`Proyecto: ${project.title}`}>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="inline-flex items-center gap-1 rounded-full border border-accent-500/20 bg-accent-500/5 px-2.5 py-0.5 text-xs font-medium text-accent-600 dark:text-accent-400">
                      {locale === 'en' ? project.statusEn : project.status}
                    </span>
                    <span className="rounded bg-gray-100 dark:bg-navy-800 px-2 py-0.5 text-xs font-mono text-gray-600 dark:text-gray-300">
                      {versions[project.id] || project.version}
                    </span>
                  </div>
                </div>
                <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2" style={{ fontFamily: 'var(--font-display)' }}>
                  {project.title}
                </h3>
                <p className="text-sm text-gray-800 dark:text-gray-100 leading-relaxed mb-5">
                ...
                <div className="flex gap-3">
                  {project.links.map((link) => (
                    <a key={link.url} href={link.url} target="_blank" rel="noopener noreferrer"
                      className="inline-flex items-center gap-2 rounded-lg border border-gray-200 dark:border-navy-600 px-4 py-2 text-sm font-medium text-gray-900 dark:text-gray-100 transition-all hover:border-gray-400 dark:hover:border-navy-400 hover:bg-gray-50 dark:hover:bg-white/5 active:scale-95">
                      {link.icon === 'github' && <GitHubIcon className="w-4 h-4" />}
                      {locale === 'en' ? link.labelEn : link.label}
                    </a>
                  ))}
