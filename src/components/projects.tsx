          ...
          <article className="glass rounded-2xl overflow-hidden card-glow mb-12">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-gray-100 dark:border-navy-700/50 px-4 py-3 sm:px-8">
              <div className="flex items-center gap-3 min-w-0">
                ...
                {/* Title always visible on mobile — no truncation */}
                <span className="text-xs font-mono text-gray-500 dark:text-navy-400 break-all sm:truncate">{featured.title.toLowerCase().replace(/\s+/g, '-')}/</span>
              </div>
              ...
              <div className="grid gap-8 lg:grid-cols-2 lg:gap-12">
                <div className="space-y-5">
                  <div>
                    ...
                    {/* Full title — no truncation, wraps on small screens */}
                    <h3 className="mt-2 text-xl font-bold text-gray-900 dark:text-white sm:text-2xl lg:text-3xl break-words" style={{ fontFamily: 'var(--font-display)' }}>{featured.title}</h3>
                  </div>
                  <p className="text-base text-gray-900 dark:text-gray-100 leading-relaxed">{locale === 'en' ? featured.longDescriptionEn : featured.longDescription}</p>
                  <div>
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {others.map(project => (
              <article key={project.id} className="glass rounded-xl p-5 card-glow flex flex-col">
                ...
                <p className="text-sm text-gray-800 dark:text-gray-200 leading-relaxed mb-4 flex-1">{locale === 'en' ? project.descriptionEn : project.description}</p>
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {project.techStack.map(tech => <span key={tech} className="rounded border border-gray-200 dark:border-navy-700/40 bg-gray-50 dark:bg-navy-800/30 px-2 py-0.5 text-xs text-gray-700 dark:text-gray-300">{tech}</span>)}
                </div>
                <div className="flex flex-wrap gap-2">
                  {project.links.map(link => (
                    <a key={link.url} href={link.url} target="_blank" rel="noopener noreferrer"
                      ...
                      {link.icon === 'github' && <GitHubIcon className="w-4 h-4" />}
                      {link.icon !== 'github' && <ExternalLinkIcon />}
                      {locale === 'en' ? link.labelEn : link.label}
                    </a>
                  ))}
                </div>
