  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 left-6 z-50 flex h-12 w-12 items-center justify-center rounded-full shadow-lg transition-all duration-300 bg-accent-500 text-white hover:bg-accent-400 ${open ? 'scale-90 ring-2 ring-accent-300' : 'hover:scale-105'}`}
        aria-expanded={open}
      ...
      >
        <div className="flex items-center justify-between border-b border-gray-100 dark:border-navy-700 px-4 py-3">
          ...
          <button type="button" onClick={reset} className="text-xs font-medium text-accent-500 hover:text-accent-600 transition-colors" aria-label={`${t.a11y.restore} opciones de accesibilidad`}>
            {t.a11y.restore}
          </button>
        </div>
            ...
            <div className="flex gap-2">
              {[0, 1, 2].map((level) => (
                <button key={level} type="button" onClick={() => update('fontSize', level)}
                  className={`flex-1 rounded-lg py-2 text-center text-xs font-semibold transition-all ${settings.fontSize === level ? 'bg-accent-500 text-black' : 'bg-gray-100 dark:bg-navy-800 text-gray-600 dark:text-navy-300 hover:bg-gray-200 dark:hover:bg-navy-700'}`}
        ...
        <div className="border-t border-gray-100 dark:border-navy-700 px-4 py-2.5">
          <p className="text-[10px] text-gray-400 dark:text-navy-500 text-center">{t.a11y.wcag}</p>
        </div>
      </div>
    </>
  );
