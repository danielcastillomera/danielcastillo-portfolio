      ...
      >
        <div className="mb-12 text-center">
          <span className="text-xs font-bold uppercase tracking-[0.2em] text-accent-500">
            {t.techGrid.sectionLabel}
          </span>
          <h2
            id="tech-heading"
            className="mt-3 text-2xl font-bold tracking-tight text-gray-900 dark:text-white sm:text-3xl"
        ...
        {TECH_GRID_CATEGORIES.map((cat) => (
          <div key={cat.key} className="mb-8 last:mb-0">
            <h3 className="text-xs font-bold uppercase tracking-wider text-gray-600 dark:text-gray-300 mb-4">
                  ...
                  <div className="w-6 h-6 shrink-0 flex items-center justify-center">
                    ...
                    <img
                      ...
                      className={`w-6 h-6 object-contain ${
                        // Next.js and Vercel logos are dark/black — invert in dark mode
                        (tech.name === 'Next.js' || tech.name === 'Vercel' || tech.name === 'Prisma' || tech.name === 'GitHub')
                          ? 'dark:invert dark:brightness-90'
                          : ''
                      } ${
                        // Express.js is also dark
                        tech.name === 'Express.js' ? 'dark:invert' : ''
                      }`}
                      loading="lazy"
                      onError={(e) => {
                        // Fallback: show first letter of tech name in a colored box
                    ...
                    <div
                      className="w-6 h-6 rounded text-white text-[10px] font-bold items-center justify-center bg-accent-500"
                      style={{ display: 'none' }}
                      aria-hidden="true"
                    >
                      {tech.name.charAt(0)}
