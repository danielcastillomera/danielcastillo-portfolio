...
export default function Contact() {
  ...
  const contactCards = [
    { icon: <WhatsAppIcon className="w-7 h-7" />, url: PERSONAL.whatsapp, title: t.contact.whatsapp, sub: t.contact.whatsappSub, hoverBorder: 'hover:border-green-500/30', hoverBg: 'hover:bg-green-50 dark:hover:bg-green-500/5', iconBg: 'bg-green-500/10', iconColor: 'text-green-500' },
    { icon: <LinkedInIcon className="w-7 h-7" />, url: PERSONAL.linkedin, title: t.contact.linkedin, sub: t.contact.linkedinSub, hoverBorder: 'hover:border-blue-500/30', hoverBg: 'hover:bg-blue-50 dark:hover:bg-blue-500/5', iconBg: 'bg-blue-500/10', iconColor: 'text-blue-600 dark:text-blue-400' },
    { icon: <GitHubIcon className="w-7 h-7" />, url: PERSONAL.github, title: t.contact.github, sub: t.contact.githubSub, hoverBorder: 'hover:border-gray-400/30', hoverBg: 'hover:bg-gray-50 dark:hover:bg-white/5', iconBg: 'bg-gray-500/10 dark:bg-white/5', iconColor: 'text-gray-700 dark:text-white' },
    { icon: <InstagramIcon className="w-7 h-7" />, url: PERSONAL.instagram, title: t.contact.instagram, sub: t.contact.instagramSub, hoverBorder: 'hover:border-pink-500/30', hoverBg: 'hover:bg-pink-50 dark:hover:bg-pink-500/5', iconBg: 'bg-pink-500/10', iconColor: 'text-pink-500' },
    { icon: <FacebookIcon className="w-7 h-7" />, url: PERSONAL.facebook, title: t.contact.facebook, sub: t.contact.facebookSub, hoverBorder: 'hover:border-blue-600/30', hoverBg: 'hover:bg-blue-50 dark:hover:bg-blue-500/5', iconBg: 'bg-blue-600/10', iconColor: 'text-blue-700 dark:text-blue-500' },
    { icon: <EmailIcon className="w-7 h-7" />, url: `mailto:${PERSONAL.email}`, title: t.contact.email, sub: PERSONAL.email, hoverBorder: 'hover:border-accent-500/30', hoverBg: 'hover:bg-amber-50 dark:hover:bg-accent-500/5', iconBg: 'bg-accent-500/10', iconColor: 'text-accent-500' },
  ];
  ...
  return (
    <section id="contacto" className="relative py-24 sm:py-32 overflow-hidden" aria-labelledby="contact-heading">
      ...
      <div ref={sectionRef} className={`relative mx-auto max-w-5xl px-6 lg:px-8 text-center transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        ...
        <p className="mt-4 text-lg text-gray-800 dark:text-gray-100 max-w-2xl mx-auto">{t.contact.subtitle}</p>
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {contactCards.map((card) => (
            <a key={card.title} href={card.url} target="_blank" rel="noopener noreferrer"
              ...
              aria-label={`${t.contact.ctaLabel} ${card.title}`}
            >
