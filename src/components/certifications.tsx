 ...
 * PdfViewer v7.6.1
 *
 * Estrategia definitiva — un solo tap, visualización inmediata:
 *
 * 1. iOS Safari             → Google Docs viewer (único método viable en iOS)
 * 2. Android Chrome/Firefox → <object> con URL directa (nativo, sin redirect)
 * 3. Desktop Chrome/Edge    → <object> nativo
 * 4. Desktop Firefox        → <embed> (fallback interno de object)
 * 5. Cualquier browser      → botón "Abrir ↗" como fallback absoluto
 *
 * CAUSA del problema anterior: se usaba Google Docs en todos los móviles.
 * Android Chrome muestra el "Abrir" intermedio porque docs.google.com es
 * un dominio externo que requiere confirmación del sistema.
 * FIX: Android Chrome recibe el PDF directo (mismo origen → sin confirmación).
 */
function PdfViewer({ file, title, onClose }: { file: string; title: string; onClose: () => void }) {
  const [zoom, setZoom] = useState(100);
  const [strategy, setStrategy] = useState<'object' | 'google-docs'>('object');
  ...
  // Detectar estrategia al montar — iOS Safari es el ÚNICO que necesita proxy
  useEffect(() => {
    const ua = navigator.userAgent;
    // iOS: iPad, iPhone, iPod con Safari (no Chrome ni Firefox en iOS)
  ...
  // Google Docs viewer — solo iOS Safari
  const googleDocsUrl = `https://docs.google.com/viewer?url=${encodeURIComponent(absoluteUrl)}&embedded=true`;

  // PDF nativo — mismo origen, sin redirect, sin confirmación del sistema
  // #toolbar=0 oculta la barra del visor del browser
  const nativePdfUrl = `${file}#toolbar=0&navpanes=0&scrollbar=1&view=FitH`;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-2.5 bg-navy-950 border-b border-navy-700 shrink-0 gap-2 flex-wrap">
        <h2 className="text-sm font-semibold text-white truncate max-w-[55%] min-w-0" title={title}>
          {title}
        </h2>
        <div className="flex items-center gap-2 shrink-0 flex-wrap">
          ...
          {/* Abrir en nueva pestaña — SIN descarga */}
          <a href={file} target="_blank" rel="noopener noreferrer"
            className="h-8 px-3 rounded-lg bg-navy-800 text-xs font-semibold text-accent-400 hover:bg-accent-500 hover:text-black transition-colors inline-flex items-center whitespace-nowrap touch-manipulation">
            Abrir ↗
          </a>
          <button type="button" onClick={onClose}
            className="h-8 px-3 rounded-lg bg-navy-800 text-xs font-semibold text-white hover:bg-red-700 transition-colors touch-manipulation"
      ...
      <div className="flex-1 overflow-auto bg-gray-900 p-2 sm:p-4 flex items-start justify-center">
        <div style={{
          ...
          {strategy === 'google-docs' ? (
            /*
             * iOS Safari: iframe con Google Docs viewer.
             * loading="eager" + autoplay para que cargue sin espera.
             */
            <iframe
              ref={iframeRef}
              src={googleDocsUrl}
              className="w-full rounded-lg shadow-2xl bg-white border-0"
              style={{ height: '82vh', minHeight: '420px', display: 'block' }}
              title={title}
              loading="eager"
             ...
             * <object> con PDF del mismo origen → sin confirmación, sin redirect.
             * El browser lo abre directamente como lo haría con una URL directa.
             */
            <object
              data={nativePdfUrl}
              type="application/pdf"
              className="w-full rounded-lg shadow-2xl bg-white"
              style={{ height: viewerH, minHeight: '420px', display: 'block' }}
              aria-label={title}
            >
              {/* Embed — fallback para Firefox/Edge sin plugin PDF.js */}
              <embed
                src={nativePdfUrl}
                type="application/pdf"
                style={{ width: '100%', height: viewerH, minHeight: '420px', display: 'block' }}
              />
              {/* Texto plano — fallback absoluto */}
              <div className="flex flex-col items-center justify-center py-16 gap-4 text-center px-6 bg-white rounded-lg min-h-[420px]">
                <p className="text-gray-700 font-semibold">Tu navegador no puede mostrar el PDF directamente.</p>
                <a href={file} target="_blank" rel="noopener noreferrer"
    ...
    <>
      <section id="certificaciones" className="relative py-24 sm:py-32" aria-labelledby="certs-heading">
        ...
        <div ref={ref} className={`relative mx-auto max-w-7xl px-6 lg:px-8 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          {/* Grid — sin imágenes ni vectores decorativos, solo texto */}
